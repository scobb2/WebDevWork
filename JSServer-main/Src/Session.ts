...
export class Session {
   ...
   // ssnsByCookie is indexed by strings
   ...
   static dltUser(user: string) {
      let ssnsId = Session.ssnsById;
      var userId: number = parseInt(String(user))
      for (let i = 0; i < ssnsId.length; i++) {
         if (ssnsId[i] !== undefined && parseInt(String(ssnsId[i].prsId))
            === userId) {
            delete Session.ssnsByCookie[ssnsId[i].authToken];
            delete ssnsId[i];
         }
      }
   };
